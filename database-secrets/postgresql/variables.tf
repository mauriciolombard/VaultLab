...
variable "vpc_id" {
  description = "VPC ID where PostgreSQL server will be deployed (use VPC from awskms-autounseal)"
  type        = string
}

variable "subnet_id" {
  description = "Subnet ID for the PostgreSQL server (optional - will use first available if not specified)"
  type        = string
  default     = ""
}
